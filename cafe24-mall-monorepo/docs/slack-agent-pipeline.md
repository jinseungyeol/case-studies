...
> 타팀의 몰 수정 요청이 "Slack DM → 담당자 복붙 → AI 처리 → 회신"으로 흐르던 **사람 중간다리를 제거**하는 자동화. 설계 → 실측 검증 → MVP 처리 엔진(접수 폼·GAS 다리·Actions 워크플로·가드 프롬프트) 구축까지 완료. MVP 범위는 2개 브랜드·최다 빈도 3유형이며, 전 브랜드·이미지 유형·앱 승인은 후속 단계.
...
| 1단계 (MVP) | 접수 폼 + 에이전트 + 결과 게시. 2개 브랜드, 최다 빈도 3유형 | ✅ 구축 완료 |
| 2단계 | 확정 워크플로 활성 + 전 브랜드 + 이미지 유형 | 예정 |
| 3단계 | 운영 실적 기반으로 커스텀 앱 승인 요청 → 스레드 대화로 업그레이드 | 조건부 |

검증 계획: 실사례를 재현한 E2E(접수→staging→결과 게시), 보완 흐름, 브랜드 잠금 가드 위반 시나리오, 승인 E2E를 각각 통과 기준과 함께 정의.
