...
타팀의 수정 요청이 "DM → 담당자 복붙 → 처리 → 회신"으로 흐르던 중간다리를 제거하는 자동화. 접수 폼 → AI 헤드리스 처리 → staging 반영 → 결과 회신 → 요청자 확정 시 자동 운영 배포까지의 흐름을 설계하고 MVP 엔진을 구축했다(2개 브랜드·최다 빈도 3유형). 상세 설계는 [slack-agent-pipeline.md](./slack-agent-pipeline.md).
